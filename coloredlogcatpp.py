...
# written originally by jeff sharkey, http://jsharkey.org/
# modified thereafter by garen j. torikian (for filtering support)
...
if arg_size >= 2:
    for i, arg in enumerate(sys.argv):
        if arg == "-f":
            filter_tags.append(sys.argv[i+1])
...
while True:
    ...
    if len(filter_tags) >= 1:
        for filter_name in filter_tags:
